    ...
    $.fn.controller = function (options) {
        ...
        let defaults = {
            text: {
                ...
                title: 'Episode XYZ',
                subtitle: 'THE JAVASCRIPT MENACE',
                scroll: 'The Galactic Republic is in turmoil as JavaScript finishes operating to become a master in it\'s trade.',
            },
            logo: {
                source: false,
                data: `<svg version="1.0" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="693.615px" height="419.375px" viewBox="0 0 693.615 419.375" enable-background="new 0 0 693.615 419.375" xml:space="preserve"> 
                    <g id="Layer_2">
                        <g>
        ...
        /**
         * append data to container elements
         * @returns void
         */
        let appendData = function () {
            if (settings.logo.source === false) {
                $(settings.elements.logo).html(settings.logo.data);
            } else {
                $(settings.elements.logo).html('<img src="' + settings.logo.source + '">');
            }
            $(settings.elements.intro).html(settings.text.intro);
            $(settings.elements.title).html(settings.text.title);
            $(settings.elements.subtitle).html(settings.text.subtitle);
            $(settings.elements.scroll).html(settings.text.scroll);
        };
        ...
        let removeStars = function () {
            $('.' + settings.stars.class).remove();
        };

        /**
         * draw stars randomly to screen
         * @returns void
         */
        let drawStars = function () {
            ...
            for (let i = 0; i < settings.stars.count; i++) {
                ...
                // get random from available styles
                let color = settings.stars.style.color[Math.floor(Math.random() * settings.stars.style.color.length)];
                let size = settings.stars.style.size[Math.floor(Math.random() * settings.stars.style.size.length)];
